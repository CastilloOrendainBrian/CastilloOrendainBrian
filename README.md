<h1 align="center">Hi! I'm Brian Castillo Orendain 👋</h1>
<h3 align="center">Web Developer | DevOps </h3>

<p align="center">
  📍 Mexico City, Mexico &nbsp;|&nbsp;
  🌐 <a href="https://borendain.com">borendain.com</a>
</p>

---

### 🚀 About Me

Software engineer with over **7 years of combined experience** in Full Stack development and infrastructure automation. I have led technical teams in the public and institutional sectors. I specialize in designing scalable backend architectures, orchestrating containerized environments, and securing production infrastructure in both cloud and self-hosted environments.

- 🛡️ Focused on defensive security: server hardening, firewalls, and compliance audits.
- 🤖 Exploring the AI ​​ecosystem applied to development: MCP, agent architecture, and advanced prompt engineering.
- 🎓 Bachelor's degree in Computer Systems Engineering and Diploma in Business Analytics & Intelligence.

---

### 🛠️ Technical Skills

**Languages & Frameworks**
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=flat-square&logo=vue.js&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)

**Databases and Storage**
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![AWS S3](https://img.shields.io/badge/AWS%20S3-569A31?style=flat-square&logo=amazons3&logoColor=white)

**Servers and DevOps**
![Ubuntu](https://img.shields.io/badge/Ubuntu%20Server-E95420?style=flat-square&logo=ubuntu&logoColor=white)
![NGINX](https://img.shields.io/badge/NGINX-009639?style=flat-square&logo=nginx&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![OpenShift](https://img.shields.io/badge/OpenShift-EE0000?style=flat-square&logo=redhatopenshift&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat-square&logo=jenkins&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Cloudflare](https://img.shields.io/badge/Cloudflare%20Tunnels-F38020?style=flat-square&logo=cloudflare&logoColor=white)

**Security and Monitoring**
`UFW` · `Fail2ban` · `SSH Hardening` · `Lynis` · `Unattended-Upgrades` · `Certbot (SSL)` · `JWT` . `Bearer Token`

**AI Ecosystem**
`MCP (Model Context Protocol)` · `Claude Code` · `Gemini CLI` · `GitHub Copilot` · `OpenCode` · `Agent Architecture`

---

💼 Notable Experience

INFOTEC — Mid-Senior Developer & Project Lead (2022–2025)
Technical leadership for internal telecommunications logistics platforms at the national level, using Laravel and Vue.js.

TotalPat — Backend / Full Stack Developer (2021)
Development of real-time procedural communication modules for the Criminal Justice System.

Independent / Freelance — Software Architect & Systems Administrator (active projects)
Design a real estate platform with Hexagonal Architecture, Laravel, Next.js, and digital signatures via DocuSign.
